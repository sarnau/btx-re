...
; Unreachable code. Four fragments, 43 bytes in total, and each one is a
; recognisable twin of something that IS reachable - which is what they are,
; rather than a mystery:
;
;   $E3EF-$E402  20 bytes  the whole-row twin of the $41 attribute handler at
;                          $E0BB. Same mask pair - $C0 on attribute byte 1, $B0
;                          on byte 3 - but through setAttrRow with span bit 0
;                          instead of applyAttr with $10, and it joins the same
;                          tail at $E47D
;   $F457-$F458   2 bytes  SEC / RTS, byte for byte what noSecondSource is, and
;                          it sits immediately in front of it
;   $FAE4-$FAF5  18 bytes  ASCII cursor-right: asciiCol up one, wrapping at 80
;                          into asciiCR and asciiLF. There is no cursor-right in
;                          asciiCtrlTable, so this is the handler the table lost
;   $FB00-$FB02   3 bytes  JMP asciiLF, byte for byte what asciiVT is, three
;                          bytes in front of asciiCurUp
;
; Each disassembles cleanly and ends properly, so they are real routines rather
; than misparsed data. Nothing in the image references any of them: no branch
; target, no table entry, and each is preceded by an instruction that ends flow.
;
