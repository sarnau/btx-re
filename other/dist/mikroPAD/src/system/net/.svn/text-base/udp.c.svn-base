...
unsigned int UDP_CloseSocket( unsigned int SOCKET )
	{
		...
		if ( UDP_sockettable[SOCKET].Socketstate == SOCKET_NOT_USE ) return( 0xffff );
			
		UDP_sockettable[SOCKET].Socketstate = SOCKET_NOT_USE;
...
unsigned int UDP_Getfreesocket()
	{
		...
		for ( Socket = 0 ; Socket < MAX_UDP_CONNECTIONS ; Socket++ ) 
			{
				if ( UDP_sockettable[ Socket ].Socketstate == SOCKET_NOT_USE ) return( Socket );
			}
		return(0xffff);
	}
//@}
