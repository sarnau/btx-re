 ...
 * \retval	IP		Die IP des Hostname, wenn IP = -1 ist war die Anfrage nicht erfolgreich. Es sollte der DNSserver Eintrag ueberprueft werden
 * oder die richtigkeit des Hostname.
 */
unsigned long DNS_ResolveName( char * HOSTNAME )
{		
		unsigned int i,UDP_socket;
		unsigned char timer;
		...
		struct DNS * DNS_question;
		DNS_question = ( struct DNS *) udpbuffer;
		...
		DNS_ans = ( struct DNS *) &DNS_question->Queries[i];
