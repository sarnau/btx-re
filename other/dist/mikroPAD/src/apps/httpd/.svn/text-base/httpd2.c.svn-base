...
void httpd_thread( void )
{
	...
	if ( http_request.HTTP_SOCKET == NO_SOCKET_USED )
	{ 	
	...
	else
	{
		...
		if( CheckSocketState( http_request.HTTP_SOCKET ) == SOCKET_NOT_USE )
		{
			...
			http_request.HTTP_SOCKET = NO_SOCKET_USED;
			http_request.STATE = DISCONNECT;
		}
		else
		{
			STDOUT_Set_TCP_Socket ( http_request.HTTP_SOCKET );
			...
			{	
				while( GetBytesInSocketData( http_request.HTTP_SOCKET ) >= 1 )
				{
					...
					if ( Data != 0x0a )
					{
						if ( http_request.HTTP_POS < REQUEST_BUFFERLEN )
						{
