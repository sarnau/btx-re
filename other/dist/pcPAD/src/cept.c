...
/* Usleep based replacement for CLOCK_delay() */
void CLOCK_delay(int time)
{
	/* Note: This has been ported from the mikroPAD firmware. The Mikrowebserver/openMCP
		 firmware offers a CLOCK_delay() function. This is the usleep based equivalent.
		 the timing will be inacurate of cause, but we hope that this will not cause
		 harmful effects */
...
int applicationBtxCeptTransmit(btxCeptPageData *page, char *port)
{
	applicationBtxTermctrlInhibit(TERMCTRL_INHIBIT_ON);
	ttyWriteLen(port, page->ceptPage, page->ceptPageLength);
...
int applicationBtxCeptGetPageRequest(char *string, char *port)
{
	...
	ttyPutchar (port, CEPT_CON);					/* Cursor on */
	...
	while((inputBuffer != '#')&&(lengthCount < dynamicMaxlength))
	{
		...
		{
			ttyPutchar (port, inputBuffer);					/* Echoplex */
		...
		{
			ttyPutchar (port, CEPT_APB);					/* Clear character from screen by overwriting it with a whitespace */
			ttyPutchar (port, ' ');					
			ttyPutchar (port, CEPT_APB);					/* Clear character from screen by overwriting it with a whitespace */
			stringPointer--;
			lengthCount--;
	...
	*stringPointer = '\0';
	...
	ttyPutchar (port, CEPT_COF);							/* Cursor off */

	return lengthCount;
}

/* Get keyboard input from the terminal (With echoplexing and the possibility of correction) */
int applicationBtxCeptGetString(char *string, int maxlength, char *port)
{
	unsigned char inputBuffer;
	char *stringPointer = string;
	int lengthCount = 0;
